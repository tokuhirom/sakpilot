...
// S3互換のデータプレーン(オブジェクト一覧・ダウンロード)はsakumock側で
// `--enable-data-plane`により提供可能だが、外部プロセス(versitygw)への委譲かつ
// control planeで発行したアクセスキーが検証されない設計のためE2Eサーバーでは
// 有効化していない(詳細はdocs/upstream-issues.md参照)。このファイルはバケット/
// アクセスキー管理のみを対象とする。
...
// 使っているが、上記の理由でE2Eサーバーはデータプレーンを有効化していないため常に
// 接続エラーになる(フロント側は空一覧として表示する)。そのため「作成したバケットが
// 一覧に反映される」までは検証できず、作成リクエスト自体がエラーなく完了することのみを
// 確認する。
