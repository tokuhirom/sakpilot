...
- 作成方法(空/ディスクコピー/アーカイブコピー)によって必須項目が変わるため、`newSource`の値に応じて名前・サイズ(GB、`min={20}`は`DiskList.tsx`のサイズ入力を踏襲)・コピー元セレクトに`required`を付与。共有キー入力は`types.ArchiveShareKey.ValidFormat()`(コロン区切り3トークン)に合わせて`pattern="[^:]+:[^:]+:[^:]+"`を付与。
- 作成モーダルの複合`disabled`条件(`!newName || (newSource === 'disk' && !newSourceDiskId) || (newSource === 'archive' && !newSourceArchiveId)`)、複製モーダルの`!sharedKeyInput || !sharedName`というrequiredと重複するJSバリデーションを削除(rule 4)。
- `ArchiveList.test.tsx`の`getByLabelText('コピー元ディスク')`を必須マーカー追加に伴い`{ exact: false }`指定に更新、名前未入力・共有キー形式不正それぞれで送信がブロックされることを確認する新規テストを2件追加。
- `tsc --noEmit`/`npm run test`(269件全パス)/`npm run build`/`golangci-lint run`(0 issues、Go側の変更なし)を確認してから作成。Archiveにはe2e specがまだ無いため今回は対象外。
- ContainerRegistryに続き、KMS(`KMSList.tsx`の作成モーダル、`KMSDetail.tsx`の基本情報インライン編集)を`docs/ui-implementation-patterns.md`のルールに合わせて改修。2フォームとも`<form onSubmit>`+`<button type="submit">`に変換した。
- 制約値は`terraform-provider-sakura`の`docs/resources/kms.md`で確認: `name`必須、`description`長さ上限512(1-512文字)、`key_origin`は任意(デフォルト`generated`)、`plain_key`は`key_origin=imported`の場合のみ必須。
- 作成モーダルの複合`disabled`条件(`!newName || (newKeyOrigin === 'imported' && !newPlainKey)`)というrequiredと重複するJSバリデーションを削除(rule 4)。
- `KMSList.test.tsx`の「インポート時にキー素材が必須」テストをボタンdisabled確認から`input.validity.valid`ベースの検証に書き換え。
- `tsc --noEmit`/`npm run test`(267件全パス)/`npm run build`+`npx playwright test e2e/kms.spec.ts`(5件全パス)/`golangci-lint run`(0 issues、Go側の変更なし)を確認してから作成。
...
- ユーザーから「今後、機能追加などするときはマニュアルの更新もする」ルール化の指示があり、CLAUDE.mdに「ユーザーマニュアルの更新」節を追加（PR #167）。あわせて非公式ツールのdisclaimerをroot `README.md`にも追記。
