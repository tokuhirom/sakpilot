...
上記以外のリソース(ディスク・スイッチ・パケットフィルター・DNS・GSLB・ProxyLB(ELB)・シンプル監視・データベース・エンハンスドDB・NFS・コンテナレジストリ・KMS・AppRun専有型/共用型 等)は、今回はサーバーのマニュアルをPOC(実装基盤の動作確認)として整備した段階です。撮影スクリプト・configはリソース非依存の共通基盤として作成済みのため、これを流用して順次追加予定です。
