...
さくらのクラウドのIAM(ユーザー・グループ・ロール・サービスプリンシパル・プロジェクト/フォルダ・組織管理・ポリシーバインディング)を利用できます。ユーザー・グループ・IAMロール・IDロールは読み取り専用の一覧表示、サービスプリンシパルとプロジェクト/フォルダは作成・編集・削除・移動、組織は名前の編集、ポリシーはバインディングの追加・編集・削除が行えます。

## 一覧画面

サイドバーの「グローバルリソース」内「IAM」から一覧に入ります。IAMは組織横断的な機能のため、他のリソースのようなゾーン/グローバルの区分ではなく単一の画面で完結します。上部のタブでユーザー・グループ・IAMロール・IDロール・サービスプリンシパル・プロジェクト/フォルダ・組織・ポリシーを切り替えます。
...
SSO/SCIM/2FA/サービスポリシー等の管理機能は今後のアップデートで対応予定です。
